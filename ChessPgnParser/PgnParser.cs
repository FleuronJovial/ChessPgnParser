...
namespace Chess.Pgn.Parser
{
    ...
    public class PgnParser
    {
        ...
        /// </returns>
        private short FindCastling(PgnGame pgnGame, PlayerColor playerColor, bool isShortCastling, ref int truncatedCount, string moveTxt, ref MoveExt move)
        {
            short retVal = -1;
            int deltaWanted;
            int delta;
        ...
        /// Moving position (Starting Position + Ending Position * 256) or -1 if error
        /// </returns>
        private short FindPieceMove(PgnGame pgnGame, PlayerColor playerColor, PieceType pieceType, int startCol, int startRow, int endPos, MoveType moveType, string moveTxt, ref int truncatedCount, ref MoveExt move)
        {
            short retVal = -1;
            List<Move> movePosList;
            int col;
            ...
            pieceType |= playerColor == PlayerColor.Black ? PieceType.Black : PieceType.White;
            movePosList = m_chessBoard.EnumMoveList(playerColor);
        ...
        /// <param name="truncatedCount"> Truncated count</param>
        /// <param name="move">           Move position</param>
        private void CnvSanMoveToPosMove(PgnGame pgnGame, PlayerColor playerColor, string moveText, out short pos, ref int truncatedCount, ref MoveExt move)
        {
            string pureMove;
            int index;
            PieceType pieceType;
            ...
            if (index != -1)
            {
                if (pureMove.Length > index + 1)
                {
                    ...
                    {
                        case 'Q':
                            moveType = MoveType.PawnPromotionToQueen;
                            break;
                        case 'R':
                            moveType = MoveType.PawnPromotionToRook;
                            break;
                        case 'B':
                            moveType = MoveType.PawnPromotionToBishop;
                            break;
                        case 'N':
                            moveType = MoveType.PawnPromotionToKnight;
                            break;
                        default:
                            pos = -1;
                            truncatedCount++;
        ...
        /// <param name="truncatedCount"> Truncated count</param>
        private void CnvSanMoveToPosMove(PgnGame pgnGame, PlayerColor colorToPlay, List<string> rawMoveList, out short[] moves, List<MoveExt>? movePosList, ref int truncatedCount)
        {
            List<short> shortMoveList;
            MoveExt move;

            move = new MoveExt(PieceType.None, 0, 0, MoveType.Normal, "", -1, -1, 0, 0);
            ...
            try
            {
                foreach (string moveTxt in rawMoveList)
                {
                    ...
                    if (pos != -1)
                    {
                        ...
                        colorToPlay = colorToPlay == PlayerColor.Black ? PlayerColor.White : PlayerColor.Black;
                    }
                    else
                    {
                        break;
        ...
        /// </returns>
        private bool ParseFen(string fenTxt, out PlayerColor colorToMove, out BoardStateMask boardStateMask, out int enPassantPos)
        {
            bool retVal = true;
            string[] cmds;
            string[] rows;
                ...
                else
                {
                    ...
                    foreach (string row in rows)
                    {
                        ...
                        foreach (char chr in row)
                        {
                                ...
                                default:
                                    if (chr is >= '1' and <= '8')
                                    {
                                        ...
                                        if (blankCount + linePos <= 8)
                                        {
                                            ...
                                            {
                                                m_chessBoard[pos--] = PieceType.None;
                                            }
                                            linePos += blankCount;
                                        }
                                    }
                            ...
                            if (retVal && pieceType != PieceType.None)
                            {
                                if (linePos < 8)
                                {
                    ...
                    if (retVal)
                    {
                        ...
                        if (cmd != "-")
                        {
                            for (int i = 0; i < cmd.Length; i++)
                            {
                                ...
                                {
                                    'K' => BoardStateMask.WRCastling,
                                    'Q' => BoardStateMask.WLCastling,
                                    'k' => BoardStateMask.BRCastling,
                                    'q' => BoardStateMask.BLCastling,
                                    _ => 0
                                };
                            }
                        }
        ...
        /// true if succeed, false if failed
        /// </returns>
        public bool ParseFen(string fenTxt, out PlayerColor startingColor, out ChessBoard? chessBoard)
        {
            bool retVal;
        ...
        private PgnGame? ParsePgn(bool isAttrList, bool isMoveList, out bool isBadMoveFound)
        {
            ...
            else
            {
                ...
                if (game[0] != '[')
                {
                    throw new PgnParserException($"Oops! Game doesn't begin with '[' Pos={pgnGame.StartingPos}");
                }
        ...
        /// </returns>
        private bool ParseIfFenLine(out PlayerColor startingColor, out ChessBoard? chessBoard)
        {
            bool retVal = false;
                                ...
                                out string? errTxt)
        {
            ...
            skipCount = 0;
            truncatedCount = 0;
            if (ParseIfFenLine(out PlayerColor startingColor, out ChessBoard? startingChessBoard))
            {
        ...
        public bool ApplySanMoveToBoard(PgnGame pgnGame, string? sanTxt, out MoveExt move)
        {
            ...
            move = new MoveExt(PieceType.None,
                               startPos: 0,
                               endPos: 0,
                               MoveType.Normal,
                               comment: "",
